...
def acados_settings_kin(Tf, N, modelparams):
    ...
    ocp.cost.cost_type = "EXTERNAL"
    #ocp.cost.cost_type_e = "EXTERNAL"
    #ocp.cost.zu = 100 * npy.ones((ocp.dims.ns,))
    #ocp.cost.zl = 100 * npy.ones((ocp.dims.ns,))
    #ocp.cost.Zu = 10 * npy.ones((ocp.dims.ns,))
    #ocp.cost.Zl = 10 * npy.ones((ocp.dims.ns,))
    #not sure if needed
    #unscale = N / Tf

    #constraints
    #stagewise  constraints for tracks with slack
    ocp.constraints.uh = npy.array([0.01])
    ocp.constraints.lh = npy.array([-1000])
    #ocp.constraints.Jsh = 1
